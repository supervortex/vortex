...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeVortexAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeVortexAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeVortexAmount = strAnonymizeVortexAmount.remove(strAnonymizeVortexAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVortexAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeVortexAmount*COIN) nMaxToAnonymize = nAnonymizeVortexAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeVortexAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeVortexAmount));
        strAnonymizeVortexAmount = strAnonymizeVortexAmount.remove(strAnonymizeVortexAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeVortexAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
