...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeVortexAmount"))
        settings.setValue("nAnonymizeVortexAmount", 1000);
    nAnonymizeVortexAmount = settings.value("nAnonymizeVortexAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeVortexAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeVortexAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeVortexAmount:
            return QVariant(nAnonymizeVortexAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeVortexAmount:
            nAnonymizeVortexAmount = value.toInt();
            settings.setValue("nAnonymizeVortexAmount", nAnonymizeVortexAmount);
            emit AnonymizeVortexAmountChanged(nAnonymizeVortexAmount);
            break;
        default:
            break;
        }
