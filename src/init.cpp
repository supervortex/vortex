...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Vortex.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Vortexd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeVortexamount=<n> " + _("Keep N Vortex anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Vortex is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Vortex is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Vortex version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Vortex") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Vortex to complete") << "\n";
    ...
    nAnonymizeVortexAmount = GetArg("-anonymizeVortexamount", 0);
    if(nAnonymizeVortexAmount > 999999) nAnonymizeVortexAmount = 999999;
    if(nAnonymizeVortexAmount < 2) nAnonymizeVortexAmount = 2;
    ...
    LogPrintf("Anonymize Vortex Amount %d\n", nAnonymizeVortexAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1VRX+1000 == (.1VRX+100)*10
       10VRX+10000 == (1VRX+1000)*10
    */
